...
const state = (p, err) => JSON.stringify({ snap: p.snapshot(), calls: p.calls, clip: p.clipboard, hist: p.history, err });
...
(async () => {
  ...
  for (let i = 0; i < VISITS; i++) {
    ...
    const ok = (label, ea, eb) => { const sa = state(A.page, ea), sb = state(B.page, eb); steps++; if (sa === sb) return true; console.log(`seed ${v.seed} (${v.kind}) differs after ${label}:${firstDiff(sa, sb)}`); return false; };
